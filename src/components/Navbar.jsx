  ...
  return (
    <div className="flex md:flex-row flex-col-reverse justify-between mb-[35px] gap-6">
      ...
      <div className="sm:flex hidden flex-row justify-end gap-4">
        <CustomButton btnType="button" title={address ? 'Create Campaign' : 'Connect Wallet'} styles={address ? 'bg-[#1dc071]' : 'bg-[#8c6dfd]'} handleClick={() => {
          ...
          <ul className="mb-4">
            {navlinks.map((Link) => (
              <li key={Link.name} className={`flex p-4 ${isActive === Link.name && 'bg-[#3a3a43]'}`} onClick={() => {
                ...
                <p className={`ml-[20px] font-serif font-semibold text-[14px] ${isActive === Link.name ? 'text-[#1dc071]' : 'text-[#808191]'}`}>
                  {Link.name}
                </p>
              </li>
            ))}
